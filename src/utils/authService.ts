...
export class AuthService {
    static getAddressSeed() {
        ...
        const jwt_aud = jwt.aud?.toString()
        return genAddressSeed(BigInt(salt!), "sub", jwt.sub!, jwt_aud!).toString();
    }
    ...
    private static async verifyPartialZkLoginSignature(zkpRequestPayload: any) {
        try {
            const proofResponse = await axios.post(PROVER_URL as string, zkpRequestPayload, {
                headers: {
    ...
    static async generateZkLoginSignature(userSignature: string): Promise<string | SerializeSignatureInput> {
    ...
    async login() {
        ...
        // console.log({ params })
        try {
            const { data } = await axios.get(OPENID_PROVIDER_URL as string);
            console.log({ data })
