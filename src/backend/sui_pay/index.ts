...
export async function transact(qr, amount, password, recipientAddress){
    let qr_wallet_address = qr.walletAddreessPublicKey;
...
export async function createCard(password, wallet_address){
    const result = await wallet.createWallet(password, wallet_address);
    const qrData = JSON.stringify(result);
