...
import { checkCardBlocked, updateTrials, createUser, setNewCard } from '../../models/db/crud';
// import { connectDB, disconnectDB } from '../../models/db/database';
...
export class Wallet {
    ...
    async createWallet(password: string, wallet_address: string) {
        try {
            ...
            console.error('Error creating wallet:', err.message || error);
            throw error;
        } 
        // finally {
        //     await disconnectDB();
    ...
    async decryptKeyphrase(encryptedData: string, password: string, amount: number, wallet_addresss: string, recipientAddress: string){
        try{
    ...
    private signAndExecuteTx = async (amt: number, phrase: string, wallet_address: string, mainWallet_address: string, recipientAddress: string) => {
